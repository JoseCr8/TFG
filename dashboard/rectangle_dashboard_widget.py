...
class RectangleDashboardCanvas(tk.Canvas):
    def __init__(self, master, width=620, height=250, **kwargs):
        super().__init__(master, width=width, height=height, **kwargs)

        # Draw elements
        self.odometer_text = self.create_text(80, 125, text="00000 m", font=("Arial", 16, "bold"))
        self.temp_text = self.create_text(520, 125, text="20°C", font=("Arial", 16, "bold"))
    ...
    def update_odometer(self, km):
        self.itemconfig(self.odometer_text, text=f"{km:05d} m")
...
class Gauge:
    def __init__(self, master: tk.Canvas, center, radius, label_left, label_right, title=None):
        ...
        self.needle = None
        self.knob = None
        self.draw_gauge(label_left, label_right, title)

    def draw_gauge(self, left_label, right_label, title):
        cx, cy = self.center
        r = self.radius
        ...
        for i in range(5):
            angle = 135 + i * 67.5
            self.draw_tick(angle)

        # Labels
        self.place_label(left_label, 135, offset=20)
        self.place_label(right_label, 405, offset=20)
